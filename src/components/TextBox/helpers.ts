...
export const validColStringToArr = (
  colourString: string,
  colType: ColourType
) => {
...
export const partialColStringToArr = (
  colourString: string,
  colType: ColourType
) => {
...
export const isValidColour = (colourString: string, colType: ColourType) => {
  ...
  if (matches !== []) {
    ...
    const validRange = matches.every((m, idx) =>
      inRange(parseInt(m), 0, ranges[idx])
    );
